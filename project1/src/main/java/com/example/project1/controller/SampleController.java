...
public class SampleController {
    ...
    // redirect 움직이지 않는 경우
    // 1) ~~DTO : 기본 공유 됨(클래스명과 동일(제일 앞글자만 소문자로))
    ...
    // 2) rttr.addFlashAttribute("이름",값) : 세션을 이용하기 때문에 따라가는 건 안 보임 ${이름}
    ...
    // public void postgetCalc1(int num1, int num2) {
    // log.info("calc 값 가져오기 ");
    // log.info("{} + {} = {}", num1, num2, (num1 + num2));

    // }

    @PostMapping("/sample/calc1")
    public void postgetCalc1(CalcDto calc, Model model) {
        log.info("calc 값 가져오기 ");
    ...
    // public void getCalc2(int num1, String op, int num2) {
    ...
    @PostMapping("/sample/calc2")
    public void getCalc2(CalcDto dto, Model model) {
        log.info("{} {} {} = {}", dto.getNum1(), dto.getNum2(), dto.getOp());
        int result = 0;
        switch (dto.getOp()) {
            case "+":
        ...
        log.info("{} {} {} = {}", dto.getNum1(), dto.getOp(), dto.getNum2(), result);
        model.addAttribute("result", result);
