    ...
    public Page<Object[]> list(String type, String keyword, Pageable pageable) {
        ...
        if (type != null) {
            ...
            if (type.contains("c")) { // 카테고리
                conditionBuilder.and(qBoard.content.contains(keyword));
            }
            if (type.contains("t")) { // 제목
                conditionBuilder.and(qBoard.title.contains(keyword));
            }
            if (type.contains("w")) { // 저자
                conditionBuilder.and(qMember.name.contains(keyword));
            }
